...
/**
 * Dedicated class for GET type rest service network calls
 * 
 * @author ga007ja
 * 
 */
public class GetRestService extends RestService {
    ...
    public GetRestService(Context context, String url,
            RequestObserver<String> requestObserver) {

        super(context, requestObserver);
        String sendUrl = url.replaceAll(" ", "%20");
        _Request = new JsonObjectRequest(Request.Method.GET, sendUrl, null,
                reponseListener, errorListener);
    }

    
    public GetRestService(Context context, String url,
            final Map<String, String> header, final OAuthService service,
            RequestObserver<String> requestObserver) {
